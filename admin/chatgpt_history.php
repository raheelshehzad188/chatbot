...
if ($admin_role == 'super_admin') {
    $query = "SELECT ch.*, a.username as admin_name FROM chatgpt_history ch 
              LEFT JOIN admins a ON ch.sub_admin_id = a.id 
              WHERE 1=1";
    $params = [];
    $types = [];
    ...
    if (!empty($phone_filter)) {
        $query .= " AND ch.phone LIKE ?";
        $params[] = "%$phone_filter%";
        $types[] = "s";
    }
    
    $query .= " ORDER BY ch.created_at DESC LIMIT 100";
...
<body>
    ...
    <div class="container">
        <div class="filters">
            <form method="GET">
                ...
                <a href="chatgpt_history.php" style="padding: 10px 20px; background: #6c757d; color: white; text-decoration: none; border-radius: 5px; display: inline-block;">Reset</a>
            </form>
        </div>
    ...
    <script>
        function showDetails(item) {
            ...
            let html = '<h2>ChatGPT Request Details</h2>';
